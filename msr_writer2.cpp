...
static unsigned char reverseHighNibble(unsigned char byte) {
    unsigned char high = (byte >> 4) & 0x0F;
    unsigned char reversedHigh = 0;
    for (int i = 0; i < 4; i++) {
        reversedHigh = (reversedHigh << 1) | (high & 1);
        high >>= 1;
    }
    return (reversedHigh << 4) | (byte & 0x0F);
}

static void reverseByteHighNibble(std::vector<unsigned char>& data, uint64_t mask) {
    for (size_t i = 0; i < data.size() && i < TRACK_DATA_SIZE; i++) {
        if ((mask >> i) & 1) {
            data[i] = reverseHighNibble(data[i]);
        }
    }
}
...
void write_card_data() {
    while (keep_running) {
        try {
            ...
            reverseByteHighNibble(latest_data.track2, masks[1]);
            reverseByteHighNibble(latest_data.track3, masks[2]);
